...
/* ===================================================================== */
/* Extension: PC-Bias TAGE Parameters                                    */
/* ===================================================================== */
struct bias_table_entry
{
    // 3-bit saturated counter: [0..7], strong not-taken = 0..1, strong taken = 6..7
    int pred; 
};

struct bias_table
{
    std::vector<bias_table_entry> entries;
};
...
// Initialize TAGE
{
    ...
    // Bias predictor table: separate small table for strongly biased branches
    // Uses 3-bit saturated counter [0..7]; strong states are 0,1 (not-taken) and 6,7 (taken)
    t_bias.entries.resize(BIAS_ENTRIES);
    for (int k = 0; k < BIAS_ENTRIES; ++k)
        t_bias.entries[k].pred = 3; // init to middle (weak/unbiased)
...
// Update using TAGE
{
    ...
    // Extension: PC-Based Bias Predictor
    // Always update bias table so it learns branch bias over time
    {
        int bidx = (pc ^ (pc >> 3)) & (BIAS_ENTRIES - 1);
        t_bias.entries[bidx].pred = taken ? sat_inc(t_bias.entries[bidx].pred, 7)
                                          : sat_dec(t_bias.entries[bidx].pred);
    }

    // If bias table handled this prediction, skip normal TAGE update
    // (don't want to pollute tagged tables with biased branches)
    if (g_used_bias)
    {
...
// Predict using TAGE
{
    ...
    // Extension: PC-Based Bias Predictor
    // Check a small dedicated table for strongly biased branches first.
    // If this branch has a strong counter (very biased), use it directly
    // and skip TAGE entirely: saves tagged table entries for hard branches.
    g_bias_idx = (pc ^ (pc >> 3)) & (BIAS_ENTRIES - 1); // slightly richer PC hash
    int bias_val = t_bias.entries[g_bias_idx].pred;
    bool bias_strong_taken    = (bias_val >= BIAS_STRONG_HI);
    bool bias_strong_nottaken = (bias_val <= BIAS_STRONG_LO);
    g_used_bias = (bias_strong_taken || bias_strong_nottaken);
    if (g_used_bias)
    {
        g_provider   = -1;
        g_prediction = bias_strong_taken;
