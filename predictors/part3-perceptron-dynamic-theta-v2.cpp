...
/* 3. Local Theta (Per-Perceptron) Table
*/
...
void perceptron_update(ADDRINT pc, bool taken)
{
    ...
    if((prediction != taken) || (abs(y) <= theta_table[index]))
    {
    ...
    */
    if(prediction != taken)
    {
        if( theta_table[index] < THETA_MAX)
        {
             theta_table[index]++;
        }
    }
    else if(abs(y) >  theta_table[index])
    {
        if( theta_table[index] > THETA_MIN)
        {
            theta_table[index]--;
        }
    }
...
void perceptron_init()
{
    ...
    theta_table.assign(NUM_PERCEPTRONS, THETA_INIT);
}
