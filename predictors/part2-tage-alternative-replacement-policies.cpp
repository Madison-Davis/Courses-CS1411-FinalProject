...
struct tagged_table_entry
{
    ...
    UINT64 last_used; // recency timestamp
};
...
// Initialize TAGE
{
    ...
    for (int i = 0; i < (int)NTABLES - 1; ++i)
    {
        ...
        for (int k = 0; k < (int)NENTRIES; ++k)
        {
            ...
            tagged_tables[i].entries[k].last_used = 0;
        }
    }
}
...
// Update using TAGE
{
    ...
    if (g_prediction != taken)
    {
        ...
        int candidates[NTABLES]; (void)candidates;
        int n_candidates = 0;
        ...
        if (n_candidates > 0)
        {
            ...
            // MAIN ALTERNATIVE REPLACEMENT POLICY UPDATE
            int chosen = start;
            ...
            for (int j = start; j < (int)NTABLES - 1; ++j)
            {
                ...
                UINT64 t = tagged_tables[j].entries[idx].last_used;
...
// Predict using TAGE
{
    ...
    for (int i = NTABLES - 2; i >= 0; --i)
    {
        ...
        {
            // update recency on ANY hit (provider OR altpred)
            tagged_tables[i].entries[idx].last_used = global_time;
