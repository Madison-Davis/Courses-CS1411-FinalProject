...
const int GHR_UPDATE_THRESHOLD = THETA/2;                       // confidence threshold for GHR updates
...
void perceptron_update(ADDRINT pc, bool taken)
{
    ...
    // EXTENSION: only update GHR if confidence is high, to reduce noise in history
    if (abs(y) > GHR_UPDATE_THRESHOLD)
    {
