...
int dynamic_theta           = THETA_INIT;                       // dynamic training threshold, starts at paper-recommended value but can adjust based on mispredictions to reduce overfitting
...
void perceptron_update(ADDRINT pc, bool taken)
{
    ...
    // EXTENSION: use dynamic theta, not a constant
    if((prediction != taken) || (abs(y) <= dynamic_theta))
    {
    ...
    /*
        Dynamic theta update
        - Wrong prediction: increase theta -> more future training
        - High-confidence correct prediction: decrease theta -> less unnecessary training
    */
    if(prediction != taken)
    {
        if(dynamic_theta < THETA_MAX)
        {
            dynamic_theta++;
        }
    }
    else if(abs(y) > dynamic_theta)
    {
        if(dynamic_theta > THETA_MIN)
        {
            dynamic_theta--;
        }
    }
